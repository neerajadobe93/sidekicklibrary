async function customFetch(originalFetch, url, data) {
  if (!url.includes("sampleform.json")) {
...
export const createFormPreview = async function (data, sitepage = null) {
  const formblock = await import(`/blocks/form/form.js`);
  const blockData = document.createElement("div");
  blockData.innerHTML = `<a href="sampleform.json"/>`;
  ...
  window.fetch = originalFetch; // Restore the original fetch
